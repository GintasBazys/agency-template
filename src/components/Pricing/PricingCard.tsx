...
const PricingCard: React.FC<PricingCardProps> = ({ plan }) => (
    <div className="relative flex flex-col p-6 mx-auto max-w-lg text-center bg-primary rounded-lg border shadow xl:p-8 text-white">
        {plan.bestChoice && (
        ...
        <strong
            className={`mb-4 ${plan.bestChoice ? "mt-3 lg:mt-0" : ""} text-2xl text-gradient font-bold`}
        >
            {plan.title}
        </strong>
        <p className="text-white sm:text-lg">{plan.description}</p>
        <div className="flex justify-center items-baseline my-8">
            <span className="mr-2 text-5xl text-white font-bold">
                {plan.price}
            </span>
